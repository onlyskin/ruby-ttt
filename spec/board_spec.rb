...
describe Board do
    ...
    describe 'winner' do
        ...
        context 'O wins in centre column' do
            it 'returns O' do
                board = board_with_moves([1, 2, 3, 5, 6, 8])
                expect(board.winner).to eq('O')
            end
        end
        context 'X wins in right column' do
            it 'returns X' do
                board = board_with_moves([3, 2, 6, 5, 9])
                expect(board.winner).to eq('X')
            end
        end
        context 'X wins in first row' do
            it 'returns X' do
                board = board_with_moves([1, 4, 2, 5, 3])
                expect(board.winner).to eq('X')
            end
        end
        context 'O wins in second row' do
            it 'returns O' do
                board = board_with_moves([1, 4, 8, 5, 9, 6])
                expect(board.winner).to eq('O')
            end
        end
        ...
        context 'X wins on first diagonal' do
            it 'returns X' do
                board = board_with_moves([1, 2, 5, 3, 9])
                expect(board.winner).to eq('X')
            end
        end
